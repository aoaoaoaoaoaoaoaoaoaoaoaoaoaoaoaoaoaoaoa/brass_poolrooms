...
mod tests {
    ...
    fn public_gauges_are_discrete_and_preserve_the_standard_default() {
        assert_eq!(
            MechanismSize::ALL.map(MechanismSize::side),
            [20.0, 24.0, 32.0]
        );
        assert_eq!(MechanismSize::default(), MechanismSize::Large);
        assert_eq!(CouplingGap::MINIMUM.points(), CouplingGap::MIN_POINTS);
    }

    #[test]
    #[should_panic(expected = "coupling gap must be finite and mechanically legible")]
    fn coupling_gap_rejects_sub_stock_spans() {
        let _gap = CouplingGap::new(CouplingGap::MIN_POINTS - 0.01);
    }

    #[test]
    fn coupling_enforces_the_canonical_gap_without_merging_responses() {
        let ctx = egui::Context::default();
        let mut rects = [Rect::ZERO; 2];
        ...
        assert_eq!(rects[0].size(), Vec2::splat(MechanismSize::Small.side()));
        assert_eq!(rects[1].left() - rects[0].right(), COUPLING_SPACING);
    }

    #[test]
    fn explicit_gap_sets_both_layout_span_and_tie_length() {
        let ctx = egui::Context::default();
        let mut rects = [Rect::ZERO; 2];
        let _frame = ctx.run_ui(RawInput::default(), |ui| {
        ...
        assert_eq!(
            ...
            CouplingGap::TIGHT.points()
        );
    }

    #[test]
    fn nested_couplings_share_one_mechanical_centerline() {
        let ctx = egui::Context::default();
        let mut centers = [0.0; 3];
        let _frame = ctx.run_ui(RawInput::default(), |ui| {
            let assembly = Coupled::horizontal(
                ui,
                |ui| {
                    crate::chrome::DragHandle::new()
            ...
            centers = [
                assembly.left.rect.center().y,
                assembly.right.left.rect.center().y,
                assembly.right.right.rect.center().y,
            ];
        });
    ...
    #[test]
    fn half_width_friction_pad_obeys_the_same_tie_gap_and_centerline() {
        let ctx = egui::Context::default();
        let mut rects = [Rect::ZERO; 2];
        let _frame = ctx.run_ui(RawInput::default(), |ui| {
            let pair = Coupled::horizontal(
                ui,
                |ui| {
                    crate::chrome::DragHandle::friction_pad()
                        .size(MechanismSize::Small)
                        .show(ui)
                },
                |ui| Monoglyph::new('×').size(MechanismSize::Small).show(ui),
